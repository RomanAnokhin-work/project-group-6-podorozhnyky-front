...
export default function ProfileStoriesClient({
  variant,
}: ProfileStoriesClientProps) {
  const { user } = useAuthStore();

  const { data, handleLoadMore, totalPages, page, isFetching } =
    usePagination<ApiStory>({
      queryKey:
        variant === "saved"
      ...
      fetchFn: async ({ page, perPage }) => {
        if (variant === "saved") {
          ...
          const items = await Promise.all(
            idsToFetch.map((id) => fetchStoryById(id)),
          );
          return {
            items,
            totalPages:
              allIds.length <= 6 ? 1 : Math.ceil((allIds.length - 6) / 3) + 1,
            page,
          };
        } else {
